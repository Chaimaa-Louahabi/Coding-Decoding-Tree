...
struct
 ...
 let%test _ = get_branche 1 arbre1 = Some (Noeud ([(1,Lettre 'e');(2,Lettre 'b')]))
  ...
  let dechiffrer  l arbre = 
    let rec sous_dechiffrer liste arbre arbre_original = 
    match liste with
    |[] -> []
    |t::q -> match (get_branche t arbre) , q with
             |None , _ -> raise CodeNonValide
             |Some(Lettre e), q -> e::(sous_dechiffrer q arbre_original arbre_original)
             |Some(Noeud(sous_l)), [] -> raise CodeNonValide
             |Some(Noeud(sous_l)), q -> sous_dechiffrer q (Noeud(sous_l)) arbre_original
    in sous_dechiffrer l arbre arbre
  ...
  let  arbre_to_liste  arbre = 
    let rec listarb_to_list list_arb = 
    match list_arb with 
    |[] -> []
    |(e, sous_arb)::q  -> 
        match sous_arb with
        |Lettre s -> [(s,[e])]@(listarb_to_list q)
        |Noeud(l) -> (List.map (fun (element, liste) -> (element,e::liste)) (listarb_to_list l))@(listarb_to_list q)
    in match arbre with 
       |Lettre e -> [(e,[])]
       | Noeud l -> listarb_to_list l
  ...
  let chiffrer  l arbre= 
    let rec sous_chiffrer l list_arbre =
    match l with
    |[] ->[]
    |t::q -> match List.assoc_opt t list_arbre with
            |None -> raise DonneeNonValide
            |Some code -> code::sous_chiffrer q list_arbre
    in List.flatten (sous_chiffrer l (arbre_to_liste arbre))
...
struct
  ...
  let chiffrer donnee arbre = 
    DRCode.recompose (Arbre.chiffrer DRDonnee.(decompose donnee) arbre)
end
